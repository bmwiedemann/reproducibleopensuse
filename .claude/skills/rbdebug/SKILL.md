...
- Test build: `cd $PACKAGEDIR && parallelism=7 parallelism2=6 rbverify` as ONE background Bash task; do other work or end the turn, the completion notification wakes you. Never poll with sleep/pgrep/tail -f. rbverify builds every `_multibuild` flavour and names the failing one; plain rbk/rbkt need `oscbuildparams="-M <flavor>"` (boost `extra`, qt6-base `docs`). Toolchain tests add `--prefer-pkgs=DIR` to oscbuildparams.
